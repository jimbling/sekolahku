        ...
        <script type="application/ld+json">
{
  ...
  "dateModified": "{{ $post->updated_at->toIso8601String() }}"
    ...
    @if (app()->environment('local'))
        @vite(['resources/css/app.css', 'resources/js/app.js', getActiveTheme() !== 'default' ? 'resources/themes/' . getActiveTheme() . '/src/app.css' : null, getActiveTheme() !== 'default' ? 'resources/themes/' . getActiveTheme() . '/src/app.js' : null, getActiveTheme() !== 'default' ? 'resources/themes/' . getActiveTheme() . '/src/pradipa.css' : null])
    @else
        <link rel="stylesheet" href="{{ asset('themes/' . getActiveTheme() . '/assets/css/app.css') }}">
        <link rel="stylesheet" href="{{ asset('themes/' . getActiveTheme() . '/assets/css/pradipa.css') }}">
        <script src="{{ asset('themes/' . getActiveTheme() . '/assets/js/app.js') }}" defer></script>
    @endif
    <!-- CSS Leaflet tanpa integrity -->
